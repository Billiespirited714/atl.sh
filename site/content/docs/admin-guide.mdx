...
- **Configuration**: Ansible (10 roles)
...
```bash
just deploy-tag staging security     # SSH, firewall, fail2ban
just deploy-tag prod services        # Web, Gemini, Gopher, Finger, games
...
| services    | Nginx, Gemini (molly-brown), Gopher (Gophernicus), Finger (efingerd), Games (Nethack, Botany) |
| ftp         | vsftpd with Explicit FTP over TLS            |
